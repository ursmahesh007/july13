        ...
        if self.action == 'list':
            ...
            if page == None or search:
                for n in queryset:
                    ...
                    #Set avg_rating to aggregated avg of ratings
                    if rating:                        
                        avg_rating = ratingquery.aggregate(Avg("rating"))['rating__avg']
                    n.avg_rating = avg_rating
                # queryset.update()
        
        #Get specific food
        if self.action == 'retrieve':
            ratingquery = models.Rating.objects.filter(recipe_id=self.kwargs['pk'])
            queryset = queryset.filter(recipe_id=self.kwargs['pk'])
            for n in queryset:
                for m in ratingquery:
                    n.avg_rating = ratingquery.aggregate(Avg("rating"))['rating__avg']
        queryset.update()
        return queryset
